import 'package:path/path.dart' as p;
import 'dart:io' as io;
import 'base_loaders.dart';

// Format:
// [-0.5, -0.5, -0.5, 0.0, 0.0, -1.0]
class VertNormLoader extends BaseLoader {
  bool loadingDataFormat1 = false;
  bool loadingIndices = false;

  final RegExp expVertNormsGroups = RegExp(
      r'\[(([\-0-9. ]+),([\-0-9. ]+),([\-0-9. ]+)),(([\-0-9. ]+),([\-0-9. ]+),([\-0-9. ]+))\]');
  final RegExp expCommentLine = RegExp(r'^\/\/');

  late AddVertexF addVertexF;
  late AddNormalF? addNormalF;
  late AddColorF? addColorF;
  late AddIndicesF? addIndicesF;

  @override
  int load(
    String dataPath,
    AddVertexF addVertex,
    AddColorF? addColor,
    AddIndicesF? addIndices,
    AddNormalF? addNormal,
  ) {
    addVertexF = addVertex;
    addNormalF = addNormal;

    var path =
        p.join(io.Directory.current.path, 'resources/objects/', dataPath);

    // Load vertices
    List<String> content = io.File(path).readAsLinesSync();

    // Comment lines
    for (var line in content) {
      if (line.isEmpty) continue;

      RegExpMatch? match = expCommentLine.firstMatch(line);
      if (match != null) {
        continue;
      }

      _loadDataFormat1(line);
    }

    return 0;
  }

  void _loadDataFormat1(String line) {
    RegExpMatch? match = expVertNormsGroups.firstMatch(line);

    if (match != null) {
      // The match has 8 groups based on the expression
      // Groups 2,3,4 and groups 6,7,8 have vertices and normals
      addVertexF(double.parse(match[2]!), double.parse(match[3]!),
          double.parse(match[4]!));
      addNormalF!(double.parse(match[6]!), double.parse(match[7]!),
          double.parse(match[8]!));
    }
  }
}
